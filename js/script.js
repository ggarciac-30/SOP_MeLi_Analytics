...
/* Dibuja las tarjetas de pregunta/respuesta dentro de <div id="analisisContainer">.
   ...
   Cada pregunta puede tener estos campos (ver data/resultados.json):
     ...
     - verCodigo   (opcional) "calculo-tu-id" -> link a una tarjeta de la seccion Calculos
...
function renderUnaTarjetaDeAnalisis(item, contenedor) {
    ...
    // --- Link "Ver codigo" (opcional) ---
    // PARA REPLICAR: en resultados.json, agrega a la pregunta el campo
    //   "verCodigo": "calculo-tu-id"
    // y en index.html, dentro de la seccion #calculos, crea (o reusa) una
    // tarjeta con ese mismo id:  <div class="codigo-card" id="calculo-tu-id">
    // El texto siempre es el mismo, el destino cambia segun el id que pongas.
    if (item.verCodigo) {
      html += '<a href="#' + item.verCodigo + '" class="ver-codigo-link">Ver el código SQL y Python completo en Cálculos →</a>';
    }
...
// Registro de instancias Chart.js activas, una por canvasId -- sin esto,
// si dibujarGrafica() se llama dos veces sobre el mismo canvas (por ejemplo,
// si copiaste una pregunta de resultados.json y olvidaste cambiar el
// canvasId de su grafica), Chart.js truena con "Canvas is already in use".
var graficasActivas = {};
...
function dibujarGrafica(g) {
  ...
  // Si ya habia una grafica dibujada en este mismo canvas, se destruye
  // primero -- evita el error "Canvas is already in use".
  if (graficasActivas[g.canvasId]) {
    graficasActivas[g.canvasId].destroy();
  }

  graficasActivas[g.canvasId] = new Chart(ctx, {
    type: g.tipo || 'bar',
    data: {
      labels: g.labels,
